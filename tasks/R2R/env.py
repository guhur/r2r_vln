...
class EnvBatch:
    ...
    def batchEpisodes(self, pathIds, scanIds, viewpointIds, headings):
        ...
        for i, (pathId, scanId, viewpointId, heading) in enumerate(
            zip(pathIds, scanIds, viewpointIds, headings)
        ):
...
def _get_panorama_states(state, sim, nav_graphs):
    ...
    try:
        adj_dict_cache, absViewIndex2points = nav_graphs[state.scanId][
        ...
        for relViewIndex in range(36):
            ...
            for point in adj_viewpointIds:
                adj_dict_cache[point]["rel_heading"] = _canonical_angle(
                    base_rel_heading + adj_dict_cache[point]["loc_rel_heading"]
                )
                adj_dict_cache[point]["rel_elevation"] = (
                    adj_dict_cache[point]["loc_rel_elevation"] + base_rel_elevation
                )
...
class R2RBatch:
    ...
    def __init__(
        ...
        self.env = EnvBatch(
            feature_store=feature_store, batch_size=batch_size, beam_size=beam_size
        )
        # self.env = envs
        ...
        for item in load_datasets(splits, encoder_type, bert_type):
            ...
            if self.att_ctx_merge == "None" or self.att_ctx_merge == "Eval":
                ...
                if tokenizer:
                    new_item["instr_encoding"].extend(instr_encoding)
        ...
        for traj in data_dict.values():
            ...
            if min_n_sentences <= 0 or self.att_ctx_merge == "None":
                ...
                for id_ix, id_perm in enumerate(
                    list(
                        combinations(
                            list(range(len(traj["instr_encoding"]))), min_n_sentences
                        )
                    )
                ):
                    new_traj = dict()
        ...
        print(
            "Average n_sentences:",
            sum([i for i in self.traj_n_sents.values()]) / len(self.traj_n_sents),
        )
    ...
    def _next_minibatch(self, sort=False):
        ...
        if sort:
            batch = sorted(
                batch,
                key=lambda item: np.argmax(item["instr_encoding"] == padding_idx),
                reverse=True,
            )
        self.batch = batch
    ...
    def _get_obs_inside(self, item, feature, state, sim):
        ...
        if self.panoramic:
            ...
            feature_with_loc_all = np.concatenate(
                (feature_all, _static_loc_embeddings[state.viewIndex]), axis=-1
            )
        ...
        if self.action_space == 6:  # todo: add discrete space single ctrl feature
            ...
            teacher = self._shortest_path_pano_action(
        ...
        if self.ctrl_feature:
            ...
            ctrl_features = self.env.feature.rollout_single(
                state.scanId,
                adj_loc_list[teacher]["nextViewpointId"],
                adj_loc_list[teacher]["absViewIndex"],
            )[1]
        ret = {
    ...
    def _get_obs(
        self, all_states
    ):  # , sub_stages):  # sub_stages: how many stages left
        obs = []
        if isinstance(all_states[0], tuple):
            ...
            for i, (sims, beam_states) in enumerate(
                zip(self.env.sims_view(), all_states)
            ):
                item = self.batch[i]
                beam_obs = []
                for sim, (feature, world_state) in zip(sims, beam_states):
                    ...
                    beam_obs.append(
                        self._get_obs_inside(item, feature, sim.getState(), sim)
                    )
                obs.append(beam_obs)
    ...
    def step(self, actions, last_obs=None, world_states=None):
        ...
        if self.action_space == 6:
            ...
            if world_states is None:
                for i, (sim, action, last_ob) in enumerate(
                    zip(self.env.sims, actions, last_obs)
                ):
                ...
                for t in zip(self.env.sims_view(), world_states, actions, last_obs):
                    ...
                    for sim, state, action, last_ob in zip(*t):
                        ...
                        feature = self.env.feature.rollout(
                            new_state.scanId,
                            new_state.location.viewpointId,
                            new_state.viewIndex,
                        )
                        world_state = WorldState(
                            scanId=new_state.scanId,
    ...
    def world_states2feature_states(
        self, world_states
    ):  # only for state_factored_search
        all_states = []
