...
class BottomUpImageAttention(nn.Module):
    ...
    def forward(self, bottom_up_features, context):
        ...
        # batch_size x max_num_detections x embedding_size
        object_embedding = self.object_embedding(bottom_up_features.object_indices)
        ...
        feats = torch.cat(
            (
                ...
                attribute_embedding,
                object_embedding,
                bottom_up_features.spatial_features,
            ),
            dim=2,
        )
...
class AttnDecoderLSTM(nn.Module):
    ...
    def forward(self, u_t_prev, all_u_t, visual_context, h_0, c_0, ctx, ctx_mask=None):
...
class SpeakerEncoderLSTM(nn.Module):
    ...
    def forward(self, batched_action_embeddings, world_state_embeddings):
        ...
        for t, (action_embedding, world_state_embedding) in enumerate(
            ...
            h, c = self._forward_one_step(h, c, action_embedding, world_state_embedding)
            h_list.append(h)
