...
def train(args, train_env, agent, log_every=log_every, val_envs=None):
    ...
    for idx in range(0, args.n_iters, log_every):
        ...
        agent.train(
            encoder_optimizer, decoder_optimizer, interval, feedback=feedback_method
        )
                    ...
                    if key not in best_metrics or best_metrics[key] < val:
                        ...
                        if not args.no_save:
                            ...
                            if key in last_model_saved:
                                for old_model_path in agent._encoder_and_decoder_paths(
...
def make_env_and_models(
    args, train_vocab_path, train_splits, test_splits, seed, test_instruction_limit=None
):
    setup()
    ...
    train_env = R2RBatch(
        image_features_list, batch_size=batch_size, splits=train_splits, tokenizer=tok
    )
    ...
    decoder = try_cuda(
        SpeakerDecoderLSTM(
            len(vocab), word_embedding_size, hidden_size, dropout_ratio, glove=glove
        )
    )
...
def train_setup(args):
    ...
    train_env, val_envs, encoder, decoder = make_env_and_models(
        args, vocab, train_splits, val_splits, args.seed
    )
    agent = Seq2SeqSpeaker(
        train_env, "", encoder, decoder, args.seed, MAX_INSTRUCTION_LENGTH
    )
