...
def batch_instructions_from_encoded(
    encoded_instructions, max_length, reverse=False, sort=False
):
    ...
    ret_tp = (
        try_cuda(Variable(seq_tensor, requires_grad=False).long()),
        try_cuda(mask.byte()),
        seq_lengths,
    )
    if sort:
...
class Seq2SeqAgent(BaseAgent):
    ...
    def _feature_variables(self, obs, beamed=False):
        ...
        feature_lists = list(
            zip(*[ob["feature"] for ob in (flatten(obs) if beamed else obs)])
        )
        assert len(feature_lists) == len(self.env.image_features_list)
        batched = []
        for featurizer, feature_list in zip(
            self.env.image_features_list, feature_lists
        ):
    ...
    def _proc_batch(self, obs, beamed=False):
        encoded_instructions = [
            ob["instr_encoding"] for ob in (flatten(obs) if beamed else obs)
        ]
        return batch_instructions_from_encoded(
            encoded_instructions,
    ...
    def _score_obs_actions_and_instructions(
        self, path_obs, path_actions, encoded_instructions
    ):
        ...
        seq, seq_mask, seq_lengths, perm_indices = batch_instructions_from_encoded(
            encoded_instructions, self.max_instruction_length, reverse=True, sort=True
        )  # self.reverse_instruction
        loss = 0
        ...
        for t in range(self.episode_len):
            ...
            target = try_cuda(
                Variable(torch.LongTensor(next_target_list), requires_grad=False)
            )
            ...
            action_scores = -F.cross_entropy(
                logit, target, ignore_index=-1, reduce=False
            ).data
    ...
    def _rollout_with_loss(self):
        ...
        for t in range(self.episode_len):
            ...
            action_scores = -F.cross_entropy(
                logit, a_t, ignore_index=-1, reduce=False
            ).data
            sequence_scores += action_scores
    ...
    def beam_search(self, beam_size, load_next_minibatch=True, mask_undo=False):
        ...
        for t in range(self.episode_len):
            ...
            all_successors = structured_map(
                lambda inf_state, world_state, obs: inf_state._replace(
                    world_state=world_state, observation=obs
                ),
                all_successors,
                successor_world_states,
                successor_obs,
    ...
    def state_factored_search(
        self,
        completion_size,
        successor_size,
        load_next_minibatch=True,
        mask_undo=False,
        first_n_ws_key=4,
    ):
        ...
        while any(len(comp) < completion_size for comp in completed):
            ...
            all_successors = structured_map(
                lambda inf_state, world_state: inf_state._replace(
            ...
            beams = structured_map(
                lambda inf_state, obs: inf_state._replace(observation=obs),
                beams,
                successor_obs,
                nested=True,
            )
    ...
    def test(
        self, use_dropout=False, feedback="argmax", allow_cheat=False, beam_size=1
    ):
