...
class Seq2SeqSpeaker(object):
    ...
    def _batch_observations_and_actions(
    ...
    def _score_obs_actions_and_instructions(
        self, path_obs, path_actions, encoded_instructions, feedback
    ):
        ...
        instr_seq, _, _ = batch_instructions_from_encoded(
            encoded_instructions, self.instruction_len, reverse=True
        )
        ...
        w_t = try_cuda(
            Variable(
                torch.from_numpy(
                    np.full((batch_size,), vocab_bos_idx, dtype="int64")
                ).long(),
                requires_grad=False,
            )
        )
        ...
        for t in range(self.instruction_len):
            ...
            word_scores = -F.nll_loss(
                log_probs, w_t, ignore_index=vocab_pad_idx, reduce=False
            )
        ...
        for t in range(self.instruction_len):
            ...
            for beam_index, beam in enumerate(beams):
                ...
                if beam:
                    for (
                        inf_index,
                        (inf_state, word_score_row, word_index_row),
                    ) in enumerate(
                        zip(
                            beam,
                            word_scores[start_index:end_index],
                            word_indices[start_index:end_index],
                        )
                    ):
            ...
            for beam_index, successors in enumerate(all_successors):
                ...
                for successor in successors:
                    if (
                        successor.last_word == vocab_eos_idx
                        or t == self.instruction_len - 1
                    ):
                ...
                this_outputs.append(
                    {
                        ...
                        "scores": scores,
                        "words": self.env.tokenizer.decode_sentence(
                            word_indices, break_on_eos=True, join=False
                        ),
