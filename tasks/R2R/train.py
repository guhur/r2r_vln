...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="path to the result_dir file",
    )
    parser.add_argument(
        "--snapshot_dir",
        dest="snapshot_dir",
        type=str,
        default="tasks/R2R/snapshots/",
        help="path to the snapshot_dir file",
    )
    parser.add_argument(
        "--plot_dir",
        dest="plot_dir",
        type=str,
        default="tasks/R2R/plots/",
        help="path to the plot_dir file",
    )

    parser.add_argument(
        "--min_count", dest="min_count", type=int, default=5, help="word min_count"
    )
    parser.add_argument(
        "--max_input_length",
        ...
        default=80,
        type=int,
        help="max_input_length",
    )

    parser.add_argument(
        "--batch_size", dest="batch_size", default=100, type=int, help="batch size"
    )
    parser.add_argument(
        "--max_episode_len",
        dest="max_episode_len",
        default=8,
        type=int,
        help="max_episode_len",
    )
    parser.add_argument(
        "--word_embedding_size",
    ...
    parser.add_argument(
        ...
        dest="action_embedding_size",
        default=64,
        type=int,
        help="action_embedding_size",
    )
    parser.add_argument(
        "--hidden_size",
        dest="hidden_size",
        default=512,
        type=int,
        help="decoder hidden_size",
    )
    parser.add_argument(
        "--enc_hidden_size",
        dest="enc_hidden_size",
        default=512,
        type=int,
        help="encoder hidden_size",
    )
    ...
    parser.add_argument(
        ...
        help="path to the image feature file",
    )
    parser.add_argument(
        "--feature_size",
        dest="feature_size",
    ...
    parser.add_argument(
        "--n_iters", dest="n_iters", default=50000, type=int, help="n_iters"
    )
    parser.add_argument(
        "--n_iters_resume",
        dest="n_iters_resume",
    ...
    parser.add_argument(
        "--n_iters_pretrain_resume",
        dest="n_iters_pretrain_resume",
        default=0,
        type=int,
        help="n_iters_pretrain_resume",
    )
    parser.add_argument(
        "--ss_n_pretrain_iters",
        dest="ss_n_pretrain_iters",
        default=-1,
        type=int,
        help="scheduled sampling n_iters in pretrain",
    )
    parser.add_argument(
        "--ss_n_iters",
        dest="ss_n_iters",
        default=-1,
        type=int,
        help="scheduled sampling n_iters",
    )
    parser.add_argument(
        "--finetune_iters",
        dest="finetune_iters",
        default=-1,
        type=int,
        help="finetune_iters for BERT",
    )
    parser.add_argument(
        "--sc_after", dest="sc_after", default=-1, type=int, help="SELF_CRITICAL_AFTER"
    )
    ...
    parser.add_argument(
        "--log_every", dest="log_every", default=100, type=int, help="log_every"
    )
    parser.add_argument(
        "--save_ckpt",
        dest="save_ckpt",
        default=-1,
        type=int,
        help="dump model checkpoint, default -1",
    )
    parser.add_argument(
        "--acc_gradient_steps",
        dest="acc_gradient_steps",
        default=1,
        type=int,
        help="the steps of accumulated gradient",
    )
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help="ratio for sample or teacher",
    )
    parser.add_argument(
        "--schedule_anneal",
        dest="schedule_anneal",
        action="store_true",
        help="schedule_ratio is annealling or not",
    )
    parser.add_argument(
        "--dropout_ratio",
        dest="dropout_ratio",
    ...
    parser.add_argument(
        ...
        default=1.0,
        type=float,
        help="temperate alpha for softmax",
    )
    parser.add_argument(
        "--learning_rate",
        dest="learning_rate",
    ...
    parser.add_argument(
        "--sc_learning_rate",
        dest="sc_learning_rate",
        default=0.00002,
        type=float,
        help="sc_learning_rate",
    )
    parser.add_argument(
        "--weight_decay",
        dest="weight_decay",
    ...
    parser.add_argument(
        "--optm", dest="optm", default="Adam", type=str, help="Adam, Adamax, RMSprop"
    )

    parser.add_argument(
        "--sc_reward_scale",
        dest="sc_reward_scale",
        default=1.0,
        type=float,
        help="sc_reward_scale",
    )
    parser.add_argument(
        "--sc_discouted_immediate_r_scale",
        dest="sc_discouted_immediate_r_scale",
        default=0.0,
        type=float,
        help="sc_discouted_immediate_r_scale",
    )
    parser.add_argument(
        "--sc_length_scale",
        dest="sc_length_scale",
        default=0.0,
        type=float,
        help="sc_length_scale",
    )

    parser.add_argument(
        "--feedback_method",
        dest="feedback_method",
        type=str,
        default="sample",
        help="sample or teacher",
    )
    parser.add_argument(
        "--bidirectional",
        dest="bidirectional",
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="6 or -1(navigable viewpoints)",
    )
    parser.add_argument(
        "--ctrl_feature",
        dest="ctrl_feature",
    ...
    parser.add_argument(
        ...
        type=str,
        default="linear",
        help="imglinear, linear, nonlinear, imgnl or deconv",
    )
    parser.add_argument(
        "--aux_n_iters",
        dest="aux_n_iters",
        type=int,
        help="update auxiliary net after aux_n_iters",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="None",
        help="mean cat mean sum (to merge attention)",
    )
    parser.add_argument(
        "--ctx_dropout_ratio",
        dest="ctx_dropout_ratio",
        type=float,
        default=0.0,
        help="ctx_dropout_ratio",
    )
    parser.add_argument(
        "--clip_gradient",
        dest="clip_gradient",
    ...
    parser.add_argument(
        ...
        type=boolean_string,
        default=True,
        help="share encoders in EncoderMultiLSTM",
    )
    parser.add_argument(
        "--decoder_init",
        dest="decoder_init",
        type=boolean_string,
        default=True,
        help="init decoder with lstm output",
    )
    parser.add_argument(
        "--dec_h_init",
        dest="dec_h_init",
    ...
    parser.add_argument(
        "--dec_h_type", dest="dec_h_type", type=str, default="none", help="none or vc"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="lstm",
        help="lstm transformer bert or gpt",
    )
    parser.add_argument(
        "--top_lstm",
        dest="top_lstm",
        type=boolean_string,
        default=False,
        help="add lstm to the top of transformers",
    )
    parser.add_argument(
        "--transformer_update",
        dest="transformer_update",
        type=boolean_string,
        default=True,
        help="update Bert",
    )
    ...
    parser.add_argument(
        "--heads", dest="heads", type=int, default=4, help="heads in transformer"
    )
    parser.add_argument(
        "--transformer_emb_size",
        dest="transformer_emb_size",
        type=int,
        default=512,
        help="transformer_emb_size",
    )
    parser.add_argument(
        "--transformer_d_ff",
        dest="transformer_d_ff",
        type=int,
        default=1024,
        help="transformer_d_ff",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train", dest="train", type=boolean_string, default=True, help="train or test"
    )

    parser.add_argument(
        "--pretrain_score_name",
        dest="pretrain_score_name",
        type=str,
        default="sr_sum",
        help="sr_sum spl_sum sr_unseen spl_unseen",
    )
    parser.add_argument(
        "--train_score_name",
        dest="train_score_name",
        type=str,
        default="sr_sum",
        help="sr_sum spl_sum sr_unseen spl_unseen",
    )
    ...
    parser.add_argument(
        ...
        type=boolean_string,
        default=False,
        help="use speaker for inference",
    )
    parser.add_argument(
        "--speaker_weight",
        dest="speaker_weight",
        ...
        help="speaker weight for inference",
    )
    parser.add_argument(
        "--speaker_prefix",
        dest="speaker_prefix",
        type=str,
        default="tasks/R2R/snapshots/release/speaker_final_release",
        help="speaker enc dec prefix",
    )
    parser.add_argument(
        "--speaker_merge",
        dest="speaker_merge",
        type=str,
        default="mean",
        help="how speaker score for multiple sentences",
    )
    parser.add_argument(
        "--state_factored",
        type=boolean_string,
        default=False,
        help="state factored beam search",
    )
    parser.add_argument(
        "--successors",
        dest="successors",
        type=int,
        default=1,
        help="successors for state_factored_search inference",
    )

    parser.add_argument("--use_pretrain", action="store_true", help="pretrain or not")
    parser.add_argument(
        "--train_in_pretrain", action="store_true", help="pretrain train or not"
    )

    parser.add_argument(
        "--pretrain_splits",
        type=str,
        default="literal_speaker_data_augmentation_paths",
        help="pretrain dataset",
    )
    parser.add_argument(
        "--pretrain_n_iters",
        dest="pretrain_n_iters",
        type=int,
        default=0,
        help="pretrain_n_iters",
    )
    parser.add_argument(
        "--pretrain_n_sentences",
        dest="pretrain_n_sentences",
        type=int,
        default=3,
    ...
    parser.add_argument(
        ...
        type=str,
        default="spl",
        help="reward function: sr_sc, spl, spl_sc, spl_last, spl_last_sc",
    )
...
assert params["panoramic"] or (
    params["panoramic"] == False and params["action_space"] == 6
)
...
TRAIN_VOCAB = os.path.join(
    SNAPSHOT_DIR, params["train_vocab"]
...
if dump_result:
    ...
    print(
        "Info: Save space mode ON. All previous best models and temporary result files will be deleted!"
    )

if ctrl_feature:
    assert aux_n_iters > 1
...
if use_glove and (glove.shape[1] != word_embedding_size):
    print(
        "Warning: reset word_embedding_size according to glove (dim=%d)"
        % glove.shape[1]
    )
...
def train(
    ...
    agent = Seq2SeqAgent(
        ...
        encoder,
        decoder,
        "resume" if n_iters_resume > 0 else 10,
        aux_ratio,
        decoder_init,
        agent_params,
        monotonic,
    ...
    if hasattr(encoder, "n_sentences"):
        encoder_n_sentences = encoder.n_sentences
    ...
    if n_iters_resume > 0:
        ...
        if len(resume_split_arr) == 0:
            enc_path = "%s%s_%s_enc_iter_%d" % (
                pretrain_model_path,
                model_prefix,
                split_string,
                n_iters_resume,
            )
            dec_path = "%s%s_%s_dec_iter_%d" % (
                pretrain_model_path,
                model_prefix,
                split_string,
                n_iters_resume,
        ...
        if hasattr(encoder, "n_sentences"):
            encoder.set_n_sentences(3)
        for env_name, (env, evaluator) in val_envs.items():
            result_path = "%s%s_%s_iter_%d.json" % (
                RESULT_DIR,
                model_prefix,
                env_name,
                n_iters_resume,
            )
            score_summary, env_loss_str, data_log = test_env(
                env_name,
                env,
                evaluator,
                agent,
        ...
        try:
            ...
            if new_best:
                print(
                    "Best score found in plot file at ",
                    best_model_iter,
                    ", best_score/best_model_iter reseted (model won't be reseted).",
                )
        except:
            pass
    ...
    if 0 < agent_params["schedule_ratio"] < 1.0:
        print(
            "Training with Scheduled Sampling, sampling ratio %.1f"
            % (agent_params["schedule_ratio"])
        )
    else:
    ...
    while idx + log_every < n_iters:
        ...
        # scheduled
        if schedule_anneal:
            agent_params["schedule_ratio"] = max(
                params["schedule_ratio"], (1.0 - float(iter) / n_iters)
            )
        ...
        if (
            (not finetune_start)
            and encoder_type == "bert"
            and (0 < finetune_iters <= iter)
        ):
        ...
        if sc_started or (SELF_CRITICAL_AFTER != -1 and iter > SELF_CRITICAL_AFTER):
            if score_name != sc_score_name:
                ...
                for env_name, (env, evaluator) in val_envs.items():
                    _, _, temp_data_log = test_env(
                        env_name,
                        env,
                        evaluator,
                        agent,
                        "%s%s_%s_iter_%d.json"
                        % (RESULT_DIR, model_prefix, env_name, best_model_iter),
                        feedback_method,
                        temp_data_log,
                        1,
                    )
        ...
        for env_name, (env, evaluator) in val_envs.items():
            score_summary, env_loss_str, data_log = test_env(
                ...
                evaluator,
                agent,
                "%s%s_%s_iter_%d.json" % (RESULT_DIR, model_prefix, env_name, iter),
                feedback_method,
                data_log,
                1,
            )
        ...
        if candidate_score > best_score:
            ...
            best_model["torch_cuda_rn"] = copy.deepcopy(
        ...
        if save_ckpt != -1 and iter % save_ckpt == 0:
            save_best_model(
                best_model, SNAPSHOT_DIR, model_prefix, split_string, best_model_iter
            )
...
def setup():
    ...
    if encoder_type != "bert" or encoder_type != "gpt":
        ...
        if not os.path.exists(TRAINVAL_VOCAB):
            write_vocab(
                build_vocab(splits=["train", "val_seen", "val_unseen"], min_count=0),
                TRAINVAL_VOCAB,
            )
...
def test_submission(opts):
    ...
    train_env = R2RBatch(
        ...
        splits=submit_splits,
        tokenizer=tok,
        att_ctx_merge=att_ctx_merge,
        bert_type=bert_type,
    )
    train_Eval = Evaluation(
        train_splits, encoder_type
    )  # , subgoal)  # jolin: add Evaluation() for reward calculation
...
def train_val(opts, n_iters_resume=0):
    ...
    train_Eval = Evaluation(
        train_splits, encoder_type
    )  # jolin: add Evaluation() for reward calculation
    ...
    if use_pretrain:
        ...
        pretrain_env = R2RBatch(
            ...
            tokenizer=tok,
            att_ctx_merge=att_ctx_merge,
            min_n_sentences=pretrain_n_sentences,
            bert_type=bert_type,
        )

        # pre-trained scheduled sampling
        if (
            agent_params["schedule_ratio"] > 0 and agent_params["schedule_ratio"] < 1.0
        ) and ss_n_pretrain_iters > 0:
            print(
                "Pretraining with Scheduled Sampling, sampling ratio %.1f"
                % (agent_params["schedule_ratio"])
            )
            ...
            print(
                "Changing to %s Pretraining with the best model at iteration %d"
                % (feedback_method, n_iters_pretrain_resume)
            )
    ...
    if 0 < agent_params["schedule_ratio"] < 1.0 and ss_n_iters > 0:
        ...
        best_model_ss_iter = train(
            ...
            ss_n_iters,
            resume_splits_array,
            cur_splits_array,  # resume_splits,
            train_score_name,
            last_train_iters if use_pretrain else -1,
            val_envs=val_envs,
            n_iters_resume=n_iters_resume,
        )
        n_iters_resume = best_model_ss_iter
        agent_params["schedule_ratio"] = -1.0
        print(
            "Changing to %s training with the best model at iteration %d"
            % (feedback_method, n_iters_resume)
        )
...
def load_test(n_iters_resume):
    ...
    if use_speaker:
        ...
        speaker_args = SpeakerArgs(
            ...
            n_iters=20000,
            no_save=False,
            result_dir=train_speaker.RESULT_DIR,
            snapshot_dir=train_speaker.SNAPSHOT_DIR,
            plot_dir=train_speaker.PLOT_DIR,
            seed=10,
    ...
    if hasattr(encoder, "n_sentences") and single_sentence_test:
        ...
        for env_name, (env, evaluator) in val_envs.items():
            json_path = "%s%s_%s_iter_%d_add.json" % (
                RESULT_DIR,
                model_prefix,
                env_name,
                n_iters_resume,
...
def test_env(
    ...
    if env_name != "test" and beam_size == 1:
        ...
        if ctrl_feature:
            if agent.decoder.ctrl_feature:
                ...
                data_log["%s loss_ctrl_f" % env_name].append(0.0)
    ...
    agent.test(
        ...
        beam_size=beam_size,
        successors=successors,
        speaker=(speaker, speaker_weight, speaker_merge, evaluator),
    )
