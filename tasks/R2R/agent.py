...
from agent_utils import (
    ...
    teacher_action,
    discount_rewards,
    backchain_inference_states,
    path_element_from_observation,
    InferenceState,
    WorldState,
    least_common_viewpoint_path,
)
...
class BaseAgent(object):
    ...
    def write_results(self, dump_file=False):
        if "_" in list(self.results.keys())[0]:
            ...
            if self.testing_settingA:
                ...
                output = [
                    {"instr_id": k, "trajectory": v[0]} for k, v in self.results.items()
                ]
            else:
                output = [
            ...
            output = [
                {"instr_id": "%s_%d" % (k, i), "trajectory": v}
                for k, v in self.results.items()
                for i in range(self.env.traj_n_sents[k])
            ]
    ...
    def test(self, beam_size=1, successors=1, speaker=(None, None, None, None)):
        ...
        batch_i, index_count = (
            0,
            [Counter() for _ in range(len(speaker_weights))] if speaker else [],
        )  # for beam search
        while True:
            ...
            for ti, traj in enumerate(trajs):
                if (beam_size == 1 and debug_beam) or (
                    beam_size > 1 and speaker is None
                ):
                    traj = traj[0]
                    ...
                    traj = speaker_rank(
                        ...
                        traj,
                        completed[ti],
                        traversed_list[ti] if traversed_list else None,
                        index_count,
                    )
                else:
        ...
        if beam_size > 1 and (speaker is not None):  # speaker's multiple choices
            ...
            for spi, speaker_weight in enumerate(speaker_weights):
                ...
                if "_" in list(self.results.keys())[0]:
                    output = [
                        {"instr_id": k, "trajectory": v[spi]}
                        for k, v in self.results.items()
                    ]
                else:
                    output = [
                        {"instr_id": "%s_%d" % (k, i), "trajectory": v[spi]}
                        for k, v in self.results.items()
                        for i in range(self.env.traj_n_sents[k])
                    ]
                ...
                print(
                    speaker_weights[spi],
                    "\n".join(
                        [str((k, round(v[0], 4))) for k, v in sorted(data_log.items())]
                    ),
                )
            ...
            print(
                ...
                best_sr,
                " speaker weight:",
                speaker_weights[best_speaker_weight_i],
            )
            print("best sr counter", index_count[best_speaker_weight_i])
            self.results = {
                k: v[best_speaker_weight_i] for k, v in self.results.items()
            }
...
def speaker_rank(
    ...
    traversed_lists,
    index_count,
):  # todo: this_completed is not sorted!! so not corresponding to beam_candidates
    cand_obs, cand_actions, multi = (
        [],
        [],
        isinstance(beam_candidates[0]["instr_encoding"], list),
    )
    cand_instr = (
        [[] for _ in beam_candidates[0]["instr_encoding"]] if multi else []
    )  # else should be np.narray
    ...
    if multi:
        ...
        speaker_scored_candidates, _ = speaker._score_obs_actions_and_instructions(
    ...
    for i, candidate in enumerate(
        beam_candidates
    ):  # different to speaker follower, our beam_candidates is not nested, we already got a subset from the outside of this function, so we do not need flatten it before enumerate
        speaker_scored_candidate = speaker_scored_candidates[i]
        if multi:
            assert candidate["instr_id"] == speaker_scored_candidate[0]["instr_id"]
            candidate["speaker_score"] = score_merge(
                [s["score"] for s in speaker_scored_candidate]
            )
        else:
...
class Seq2SeqAgent(BaseAgent):
    ...
    def _sort_batch(self, obs):
        ...
        if isinstance(sorted_tensor, list):
            ...
            for i in range(len(sorted_tensor)):
                sorted_tensors.append(
                    Variable(sorted_tensor[i], requires_grad=False).long().to(device)
                )
        ...
        return (
            Variable(sorted_tensor, requires_grad=False).long().to(device),
            mask.byte().to(device),
            list(seq_lengths),
            list(perm_idx),
        )
    ...
    def _feature_variable(self, obs):
        ...
        if isinstance(obs[0]["feature"], tuple):  # todo?
            features_pano = np.repeat(
                np.expand_dims(
                    np.zeros_like(obs[0]["feature"][0], dtype=np.float32), 0
                ),
                len(obs),
                axis=0,
            )  # jolin
            features = np.repeat(
                np.expand_dims(
                    np.zeros_like(obs[0]["feature"][1], dtype=np.float32), 0
                ),
                len(obs),
                axis=0,
            )  # jolin
            ...
            return (
                Variable(torch.from_numpy(features_pano), requires_grad=False).to(
                    device
                ),
            ...
            features = np.repeat(
                np.expand_dims(np.zeros_like(obs[0]["feature"], dtype=np.float32), 0),
                len(obs),
                axis=0,
            )  # jolin
            for i, ob in enumerate(obs):
    ...
    def _action_variable(self, obs):
        ...
        action_embeddings = np.zeros(
            (len(obs), max_num_a, action_embedding_dim), dtype=np.float32
        )
        for i, ob in enumerate(obs):
            adj_loc_list = ob["adj_loc_list"]
            num_a = len(adj_loc_list)
            is_valid[i, 0:num_a] = 1.0
        ...
        return (
            Variable(torch.from_numpy(action_embeddings), requires_grad=False).to(
                device
            ),
    ...
    def _teacher_action(self, obs, ended):
        a = teacher_action(
            self.model_actions, self.decoder.action_space, obs, ended, self.ignore_index
        )
    ...
    def _teacher_feature(self, obs, ended):  # , max_num_a):
        ...
        return Variable(torch.from_numpy(ctrl_features_tensor), requires_grad=False).to(
            device
        )
    ...
    def rollout(self, beam_size=1, successors=1):
        ...
        with torch.no_grad():
            if self.state_factored:
                beams = self.state_factored_search(
                    beam_size, successors, first_n_ws_key=4
                )
            else:
    ...
    def state_factored_search(self, completion_size, successor_size, first_n_ws_key=4):
        ...
        seq, seq_mask, seq_lengths, perm_idx = self._sort_batch(
            [o for ob in initial_obs for o in ob]
        )

        world_states = [
            [world_state for f, world_state in states] for states in world_states
        ]
        ...
        beams = [
            [
                inf_state
                for world_state, (inf_state, expanded) in sorted(instance_cache.items())
            ]
            for instance_cache in state_cache
        ]  # sorting is a noop here since each instance_cache should only contain one
        ...
        def update_traversed_lists(new_visited_inf_states):
            ...
            for instance_index, instance_states in enumerate(new_visited_inf_states):
                ...
                assert (
                    last_expanded.world_state.viewpointId
                    == traversed_lists[instance_index][-1].world_state.viewpointId
                )
                for inf_state in instance_states:
                    ...
                    assert (
                        path_from_last_to_next[-1].world_state.viewpointId
                        == inf_state.world_state.viewpointId
                    )
        ...
        while any(len(comp) < completion_size for comp in completed):
            ...
            h_t, c_t, alpha, logit, pred_f = self.decoder(
                None,
                u_t_prev,
                u_t_features,
                f_t,
                ...
                h_t,
                c_t,
                [ctx_si[beam_indices] for ctx_si in ctx]
                if isinstance(ctx, list)
                else ctx[beam_indices],
                [seq_mask_si[beam_indices] for seq_mask_si in seq_mask]
                if isinstance(ctx, list)
                else seq_mask[beam_indices],
            )
            ...
            all_successors = []
            for beam_index, (beam, beam_world_states) in enumerate(
                zip(beams, world_states)
            ):
                successors = []
                ...
                if beam:
                    for (
                        inf_index,
                        (inf_state, world_state, action_score_row),
                    ) in enumerate(
                        zip(beam, beam_world_states, log_probs[start_index:end_index])
                    ):
                        ...
                        for action_index, action_score in enumerate(action_score_row):
                            ...
                            successors.append(
                                InferenceState(
                                    ...
                                    last_action_embedding=u_t_features[
                                        flat_index, action_index
                                    ].detach(),
                                    ...
                                    c_t=c_t[flat_index],
                                    last_alpha=[
                                        alpha_si[flat_index].data for alpha_si in alpha
                                    ]
                                    if isinstance(alpha, list)
                                    else alpha[flat_index].data,
                                )
                            )
            ...
            successor_world_states = [
                [world_state for f, world_state in states]
                for states in successor_world_states
            ]
            ...
            for beam_index, (successors, instance_cache) in enumerate(
                zip(all_successors, state_cache)
            ):
                ...
                for successor in successors:
                    ...
                    if (
                        successor.last_action == 0
                        or successor.action_count == self.episode_len
                    ):
                        if (
                            ws_keys not in instance_completed_holding
                            or instance_completed_holding[ws_keys][0].score
                            < successor.score
                        ):
                            instance_completed_holding[ws_keys] = (successor, False)
                    else:
                        if (
                            ws_keys not in instance_cache
                            or instance_cache[ws_keys][0].score < successor.score
                        ):
                            instance_cache[ws_keys] = (successor, False)

                # third value: did this come from completed_holding?
                uncompleted_to_consider = (
                    (ws_keys, inf_state, False)
                    for (ws_keys, (inf_state, expanded)) in instance_cache.items()
                    if not expanded
                )
                completed_to_consider = (
                    (ws_keys, inf_state, True)
                    for (
                        ws_keys,
                        (inf_state, expanded),
                    ) in instance_completed_holding.items()
                    if not expanded
                )
                import itertools
                import heapq
                ...
                for ws_keys, inf_state, is_completed in ws_keys_and_inf_states:
                    if is_completed:
                        ...
                        if (
                            ws_keys not in instance_completed
                            or instance_completed[ws_keys].score < inf_state.score
                        ):
            ...
            world_states = [
                [inf_state.world_state for inf_state in beam] for beam in beams
            ]
            successor_obs = np.array(
                self.env._get_obs(self.env.world_states2feature_states(world_states))
            )
        ...
        for this_completed in completed:
            completed_list.append(
                sorted(this_completed.values(), key=lambda t: t.score, reverse=True)[
                    :completion_size
                ]
            )
        completed_ws = [
            [inf_state.world_state for inf_state in comp_l] for comp_l in completed_list
        ]
        completed_obs = np.array(
            self.env._get_obs(self.env.world_states2feature_states(completed_ws))
        )
        accu = []
        ...
        for this_completed in completed_list:
            ...
            for inf_state in this_completed:
                (
                    ...
                    path_scores,
                    path_attentions,
                ) = backchain_inference_states(inf_state)
                ...
                trajectory = [
                    path_element_from_observation(ob) for ob in path_observations
                ]
                this_trajs.append(
                    {
    ...
    def beam_search(self, beam_size):
        ...
        for t in range(self.episode_len):
            ...
            assert len(beams) == len(world_states)
            all_successors = []
            for beam_index, (beam, beam_world_states, beam_obs) in enumerate(
                zip(beams, world_states, obs)
            ):
                successors = []
                end_index = start_index + len(beam)
                assert len(beam_obs) == len(beam) and len(beam_world_states) == len(
                    beam
                )
                if beam:
                    for (
                        ...
                        zip(
                            ...
                            beam_world_states,
                            beam_obs,
                            action_scores[start_index:end_index],
                            action_indices[start_index:end_index],
                        )
                    ):
                        flat_index = start_index + inf_index
                        for action_score, action_index in zip(
                            action_score_row, action_index_row
                        ):
                            if is_valid_numpy[flat_index, action_index] == 0:
                                continue
                            successors.append(
            ...
            for ttt in zip(all_successors, successor_world_states, successor_obs):
                mapped = [
                    inf._replace(world_state=ws, observation=o)
                    for inf, ws, o in zip(*ttt)
                ]
                acc.append(mapped)
            all_successors = acc
            ...
            obs = [[inf_state.observation for inf_state in beam] for beam in beams]
        ...
        for this_completed in completed:
            assert this_completed
            this_trajs = []
            for inf_state in sorted(
                this_completed, key=lambda t: t.score, reverse=True
            )[:beam_size]:
                (
                    path_states,
                    path_observations,
    ...
    def rollout_with_loss(self):
        ...
        for t in range(self.episode_len):
            ...
            h_t, c_t, alpha, logit, pred_f = self.decoder(
                a_t_prev, u_t_prev, u_t_features, f_t, f_t_all, h_t, c_t, ctx, seq_mask
            )
            ...
            # Auxiliary training
            if self.decoder.ctrl_feature:
                target_f = self._teacher_feature(
                    perm_obs, ended
                )  # , is_valid.shape[-1])
                self.loss_ctrl_f += self.aux_ratio * self.criterion_ctrl_f(
                    pred_f, target_f
                )
            ...
            if self.schedule_ratio >= 0 and self.schedule_ratio <= 1:
                sample_feedback = random.choices(
                    ["sample", "teacher"],
                    [self.schedule_ratio, 1 - self.schedule_ratio],
                    k=1,
                )[
                    0
                ]  # schedule sampling
            ...
            if self.testing_settingA:
                log_probs = F.log_softmax(logit, dim=1).data
                action_score = (
                    log_probs[torch.arange(batch_size), a_t].cpu().data.numpy()
                )
        ...
        for iter in range(1, n_iters + 1):
            ...
            if self.clip_gradient_norm > 0:  # clip gradient norm
                torch.nn.utils.clip_grad_norm_(
                    self.encoder.parameters(), self.clip_gradient_norm
                )
                torch.nn.utils.clip_grad_norm_(
                    self.decoder.parameters(), self.clip_gradient_norm
                )
    ...
    def rl_rollout(
        ...
        for t in range(self.episode_len):
            ...
            # Decoding actions
            h_t, c_t, alpha, logit, pred_f = decoder(
                a_t_prev, u_t_prev, u_t_features, f_t, f_t_all, h_t, c_t, ctx, seq_mask
            )
    ...
    def rl_train(
        ...
        n_iters,
        sc_reward_scale,
        sc_discouted_immediate_r_scale,
        sc_length_scale,
    ):
        ...
        for iter in range(1, n_iters + 1):  # n_iters=interval
            ...
            seq, seq_mask, seq_lengths, perm_idx = self._sort_batch(obs)
            perm_obs = obs[perm_idx]
            gen_traj, mask, seqLogprobs, gen_results = self.rl_rollout(
                obs,
                perm_obs,
                seq,
                seq_mask,
            ...
            obs = np.array(
                self.env._get_obs(world_states)
            )  # for later 'sample' feedback batch
            perm_obs = obs[perm_idx]
            ...
            with torch.no_grad():
                greedy_traj, _, _, greedy_res = self.rl_rollout(
                    obs,
                    perm_obs,
                    seq,
                    seq_mask,
    ...
    def get_self_critical_reward(
        ...
        ave_steps = (np.array(gen_scores["trajectory_steps"])).sum() / float(
            len(instr_id_order)
        )
        ...
        if self.reward_func == "sr_sc":
            reward = (
                np.repeat(hits[:, np.newaxis], self.episode_len, 1) * sc_reward_scale
            )
        elif self.reward_func == "spl":
            reward = (
                np.repeat(gen_spls[:, np.newaxis], self.episode_len, 1)
                * sc_reward_scale
            )
            ...
            reward = (
                np.repeat(spl_sc[:, np.newaxis], self.episode_len, 1) * sc_reward_scale
            )
            ...
            tj_steps = [s - 1 for s in gen_scores["trajectory_steps"]]  # tj_steps
            reward = np.zeros((gen_spls.shape[0], self.episode_len), dtype=float)
            reward[range(gen_spls.shape[0]), tj_steps] = [
                x - y for x, y in zip(gen_scores["spl"], greedy_scores["spl"])
            ]
        elif self.reward_func == "spl_psc":  # test
            tj_steps = [s - 1 for s in gen_scores["trajectory_steps"]]  # tj_steps
            reward = np.full(
                (gen_spls.shape[0], self.episode_len), -sc_length_scale, dtype=float
            )  # penalty
            reward[range(gen_spls.shape[0]), tj_steps] = gen_scores["spl"]
        ...
        if sc_discouted_immediate_r_scale > 0:
            discounted_r = (
                discount_rewards(gen_scores["immediate_rewards"], self.episode_len)
                * sc_discouted_immediate_r_scale
            )
        ...
        if sc_length_scale:
            length_panelty = (
                np.repeat(gen_lengths[:, np.newaxis], self.episode_len, 1)
                * sc_length_scale
            )
        ...
        while write_num < 10:
            try:
                ...
                if torch.cuda.is_available():
                    torch.save(
                        torch.cuda.random.get_rng_state(), decoder_path + ".rng.gpu"
                    )
    ...
    def load(self, encoder_path, decoder_path):
        ...
        self.encoder.load_state_dict(
            torch.load(encoder_path, "cuda:0" if torch.cuda.is_available() else "cpu")
        )
        self.decoder.load_state_dict(
            torch.load(decoder_path, "cuda:0" if torch.cuda.is_available() else "cpu"),
            strict=False,
        )
