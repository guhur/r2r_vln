...
class Evaluation(object):
    ...
    def _score_item(self, instr_id, path):
        ...
        spl = (
            0.0
            if nav_error >= self.error_margin
            else (float(sp_length) / max(trajectory_length, sp_length))
        )
    ...
    def score_output(self, items):
        ...
        score_summary = {
            ...
            "oracle_rate": float(
                sum(self.scores["oracle_success"]) / len(self.scores["oracle_success"])
            ),
    ...
    def score_batch(self, batch, instr_id_order):  # jolin
        ...
        for instr_id in instr_id_order:
            ...
            distance_left = [
                self.distances[gt["scan"]][point[0]][goal] for point in path
            ]
...
def eval_seq2seq():  #  , subgoal=False):
    ...
    outfiles = [
        ...
        RESULT_DIR
        + "seq2seq_sample_imagenet_%s_iter_122000.json"
    ]
