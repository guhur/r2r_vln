    ...
    for split in splits:
        ...
        if encoder_type in ["bert", "gpt"]:
            ...
            if encoder_type == "bert" and bert_type == "large":
                filename = "tasks/R2R/data/R2R_%s_%s_%s.txt" % (
                    split,
                    encoder_type,
                    bert_type,
                )
...
class SplitTokenizer:
    ...
    def encode_sentence(self, sentence):
        ...
        encoding = (
            ...
            else [int(i) for i in sentence.strip().split("_")]
        )
...
def preprocess_get_pano_states(
...
def show_path_steps_len(splits):
    ...
    plt.hist(
        path_lens, bins=[i for i in range(min(path_lens), max(path_lens) + 1)]
